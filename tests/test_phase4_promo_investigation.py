...
This test now validates an important observed Shopee behavior:
...
def capture_navigation_get_pc(browser_session, label, target_url, run_dir):
    ...
    Unlike the previous implementation, this does not require get_pc to be
    the one response emitted immediately after navigation. We record the
    complete response URL/status sequence first, then parse any get_pc
    response that actually occurred.
    """
    ...
    for item in get_pc_responses:
        parsed = ShopeeAPIParser().parse(item["data"])
        get_pc_parsed.append(
            {
                "status": item["status"],
                "url": item["url"],
                "product": parsed,
    ...
    return final_url, response_records, get_pc_parsed
...
def main(args):
    ...
    try:
        ...
        # ---------------------------------------------------------------
        # A: Promotional URL network observation
        # ---------------------------------------------------------------
        ...
        if promo_get_pc:
            product = promo_get_pc[-1]["product"]
            print(
            ...
            product = None
            print(
                "[PHASE4] Promotional URL produced NO get_pc response "
                "during the observation window."
            )

        # ---------------------------------------------------------------
        # B: Canonical URL control observation
        # ---------------------------------------------------------------
        ...
        # Prefer promotional ProductInfo when available; otherwise use the
        # canonical control so the test can still validate live SKU mapping.
        product = product or canonical_product
        if product is None:
            raise RuntimeError(
                "Neither promotional nor canonical PDP navigation produced "
                "a parseable get_pc response. See api/*_responses.json for "
                "the captured network evidence."
            )
        ...
        product_option_keys = sorted(
            {key for variation in product.available_variations for key in variation.options}
        )
        ...
        # ---------------------------------------------------------------
        # C: Compare the exact model across any captured get_pc responses.
        # ---------------------------------------------------------------
        sku_parser = SkuPriceParser()
        sku_observations = []
        all_get_pc = promo_get_pc + canonical_get_pc

        for source in all_get_pc:
            try:
                ...
                # The ProductInfo parser above is the authoritative model
                # resolution. Raw get_pc files remain available for deeper
                # inspection; do not fail the test on parser-shape variance.
                state = None
            ...
            sku_observations.append(
                {
                    ...
                    "contains_target_model": any(
                        item.model_id == variation.model_id
                        for item in source["product"].available_variations
                    ),
                }
            )

        manifest["sku_observations"] = sku_observations
        manifest["promotional_vs_canonical"] = {
            "promotional_get_pc": len(promo_get_pc),
            "canonical_get_pc": len(canonical_get_pc),
        ...
        print(
            ...
            "PASS" if promo_get_pc else "NO get_pc",
        )
        print(
            "Canonical navigation:   ",
            "PASS" if canonical_get_pc else "NO get_pc",
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Retained for compatibility; Phase 4 uses a fixed short navigation observation window.",
    )
