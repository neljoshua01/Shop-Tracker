...
This test validates the promotional/canonical PDP path and then performs a
read-only cart investigation. The cart step is intentionally navigation-only:
it records cart API JSON and inspects the cart DOM for the exact item/model so
we can determine why the production checkout flow previously fell back to
...
def capture_navigation_get_pc(browser_session, label, target_url, run_dir):
    ...
    save_json(run_dir / "api" / f"{label}_responses.json", response_records)
...
def _walk_cart_identity(node, path="root", results=None):
    """Find cart API objects that expose item/model identity.

    This is deliberately schema-agnostic. Shopee cart payloads have changed
    shape over time, so the test records every object containing item_id/itemid
    and/or model_id/modelid instead of assuming one production API structure.
    """
    if results is None:
        results = []
    ...
    if isinstance(node, dict):
        ...
        item_value = normalized.get("item_id", normalized.get("itemid"))
        model_value = normalized.get("model_id", normalized.get("modelid"))
        shop_value = normalized.get("shop_id", normalized.get("shopid"))

        if item_value is not None or model_value is not None:
            results.append(
                {
                    "path": path,
                    "item_id": item_value,
                    ...
                    "promotion_id": normalized.get("promotion_id")
                    or normalized.get("promotionid"),
                    "promotion_type": normalized.get("promotion_type")
                    or normalized.get("promotiontype"),
                }
            )

        for key, value in node.items():
            _walk_cart_identity(value, f"{path}.{key}", results)

    elif isinstance(node, list):
        for index, value in enumerate(node):
            _walk_cart_identity(value, f"{path}[{index}]", results)

    return results


def capture_cart_evidence(browser_session, session_item_id, session_model_id, run_dir):
    """Navigate to cart only and capture API + DOM identity evidence.

    No checkbox, cart button, checkout button, or order button is clicked.
    """
    runtime = AsyncRuntime.instance()
    page = browser_session.page
    captured = []
    ...
    async def _capture():
        ...
        try:
            ...
            for response in captured:
                ...
                identities = _walk_cart_identity(data)
                matching = []
                for identity in identities:
                    item_match = str(identity.get("item_id")) == str(session_item_id)
                    model_match = str(identity.get("model_id")) == str(session_model_id)
            ...
            return page.url, api_records, api_identity_matches
        finally:
            try:
                page.remove_listener("response", on_response)
            except Exception:
    ...
    try:
        future = runtime.submit(_capture())
        final_url, api_records, api_identity_matches = future.result(timeout=50)
    except Exception as exc:
        raise RuntimeError(f"Could not inspect cart: {exc!r}") from exc

    save_json(run_dir / "api" / "cart_responses.json", api_records)
    save_json(
        run_dir / "api" / "cart_identity_matches.json",
        api_identity_matches,
    )

    actions = BrowserActions(browser_session)
    checkbox_count = actions.count(
        actions.find_all("input.stardust-checkbox__input")
    )
    dom_matches = []
    ...
    for index in range(checkbox_count):
        ...
        for level in range(1, 9):
            ...
            item_match = str(session_item_id) in " ".join(
                list(attrs.values()) + [text]
            )
            model_match = str(session_model_id) in " ".join(
                list(attrs.values()) + [text]
            )
    ...
    result = {
        ...
        "item_id": int(session_item_id),
        "model_id": int(session_model_id),
        "api_response_count": len(api_records),
        "api_identity_matches": api_identity_matches,
        "dom_checkbox_count": checkbox_count,
        "dom_identity_matches": dom_matches,
        "exact_api_item_model_match": any(
            match.get("item_match") and match.get("model_match")
            for response in api_identity_matches
            for match in response["matches"]
        ),
        "exact_dom_item_model_match": any(
            match.get("item_match") and match.get("model_match")
    ...
    print(f"[PHASE4] Cart API responses: {len(api_records)}")
    print(
        f"[PHASE4] API exact item+model match: "
        f"{'YES' if result['exact_api_item_model_match'] else 'NO'}"
    )
    print(f"[PHASE4] Cart checkboxes: {checkbox_count}")
    print(
        f"[PHASE4] DOM exact item+model match: "
        f"{'YES' if result['exact_dom_item_model_match'] else 'NO'}"
    )
...
def main(args):
    ...
    try:
        ...
        # B: Observe the canonical URL as a control.
        ...
        manifest["diagnosis"] = {
            ...
            "promotional_resolved_to_canonical": (
                ...
                and str(ITEM_ID) in promo_final_url
                and str(SHOP_ID) in promo_final_url
            ),
        }

        # C: Read-only cart investigation. We do NOT prepare the cart and do
        # NOT click Add to Cart; the goal here is to inspect whatever cart
        # state already exists and determine whether the exact model identity
        # is exposed by API and/or DOM.
        cart_result = capture_cart_evidence(
            browser_session,
            ITEM_ID,
            variation.model_id,
            run_dir,
        ...
        print(
            "Cart API exact model:  ",
            "PASS" if cart_result["exact_api_item_model_match"] else "NOT PROVEN",
        )
        print(
            "Cart DOM exact model:  ",
            "PASS" if cart_result["exact_dom_item_model_match"] else "NOT PROVEN",
        )
