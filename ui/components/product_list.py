...
class ProductList(ctk.CTkScrollableFrame):
    def __init__(self, master, stop_callback=None, **kwargs):
        ...
        columns = ["Product", "Stock", "Auto Checkout", "Target Price", "Current Price", "Last Checked", "Actions"]
        ...
        for index, label in enumerate(columns):
            ctk.CTkLabel(
                ...
                anchor="w",
            ).grid(row=0, column=index, sticky="w", padx=(10 if index == 0 else 6, 4))
        ...
        self.empty_icon = ctk.CTkLabel(
            self.empty_state,
            text="▣",
            font=(fonts.FONT_FAMILY, 23),
            text_color=colors.INFO,
        )
        self.empty_icon.pack(pady=(0, 5))
        ...
        self.empty_description.pack(pady=(3, 0))
    ...
    def update_product(self, product):
        ...
        if product.url in self.cards:
            ...
            return
        card = ProductCard(self, product, stop_callback=self.stop_callback, set_target_callback=self.set_target_callback)
