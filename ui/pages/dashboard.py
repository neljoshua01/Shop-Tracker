...
class DashboardPage(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        header_row.grid(row=0, column=0, sticky="ew", padx=content_pad, pady=(15, 8))
        ...
        self.topbar.grid(row=1, column=0, sticky="ew", padx=content_pad, pady=(0, 8))

        self.status_bar = StatusBar(self)
        self.status_bar.grid(row=2, column=0, sticky="ew", padx=content_pad, pady=(0, 10))

        self.stats = DashboardStats(self)
        self.stats.grid(row=3, column=0, sticky="ew", padx=content_pad, pady=(0, 12))

        workspace = ctk.CTkFrame(self, fg_color="transparent")
        workspace.grid(row=4, column=0, sticky="nsew", padx=content_pad, pady=(0, 12))
        ...
        bottom.grid(row=5, column=0, sticky="ew", padx=content_pad, pady=(0, 10))
    ...
    def build_engine_status(self, parent):
        engine_status = ctk.CTkFrame(
            parent, fg_color=colors.TOPBAR, border_width=1,
            border_color=colors.BORDER, corner_radius=8, width=190, height=48
        )
        engine_status.grid(row=0, column=1, sticky="e", padx=(16, 0))
        engine_status.grid_propagate(False)

        ctk.CTkFrame(
            engine_status, width=10, height=10,
            fg_color=colors.SUCCESS, corner_radius=5
        ).place(x=14, y=19)
        ctk.CTkLabel(
            engine_status, text="Engine Status", font=fonts.SMALL_BOLD,
            text_color=colors.TEXT_PRIMARY
        ).place(x=34, y=8)
        ctk.CTkLabel(
            engine_status, text="All Systems Operational", font=fonts.SMALL,
            text_color=colors.TEXT_MUTED
        ).place(x=34, y=25)

    def panel(self, parent, row, column, title, subtitle=None):
        frame = ctk.CTkFrame(
            parent, fg_color=colors.CARD, border_width=1,
    ...
    def build_engine_panel(self, parent):
        ...
        modules.pack(fill="x", padx=14, pady=(0, 12))
        ...
        for index, (title, text, icon, accent) in enumerate(stages):
            card = ctk.CTkFrame(
                modules, fg_color=colors.SURFACE, border_width=1,
                border_color=colors.DIVIDER, corner_radius=7, height=58
            )
            card.grid(row=0, column=index, sticky="ew", padx=(0, 8) if index < 2 else 0)
            card.grid_propagate(False)

            icon_box = ctk.CTkFrame(
                card, width=38, height=38,
            ...
            ctk.CTkLabel(card, text=title, font=fonts.SMALL_BOLD, text_color=colors.TEXT_PRIMARY).place(x=56, y=8)
            ctk.CTkLabel(card, text=text, font=fonts.SMALL, text_color=colors.TEXT_MUTED).place(x=56, y=29)
    ...
    def build_pipeline_panel(self, parent):
        ...
        for index, (name, detail, accent) in enumerate(stages):
            row = ctk.CTkFrame(
                ...
                border_color=colors.DIVIDER, corner_radius=7, height=43
            )
            row.pack(fill="x", pady=(0, 3))
            row.pack_propagate(False)
            ctk.CTkFrame(row, width=3, height=25, fg_color=accent, corner_radius=2).place(x=9, y=9)
            ctk.CTkLabel(row, text=name, font=fonts.SMALL_BOLD, text_color=colors.TEXT_PRIMARY).place(x=20, y=7)
            ctk.CTkLabel(row, text=detail, font=fonts.SMALL, text_color=colors.TEXT_MUTED).place(x=20, y=24)
            if index < len(stages) - 1:
                ctk.CTkLabel(
                    pipeline, text="↓", font=(fonts.FONT_FAMILY, 11),
                    text_color=colors.TEXT_MUTED
                ).pack(pady=(0, 1))
    ...
    def build_alerts_panel(self, parent):
        ...
        ctk.CTkLabel(empty, text="No active alerts", font=fonts.SUBTITLE, text_color=colors.TEXT_PRIMARY).pack(pady=(16, 3))
        ctk.CTkLabel(
            empty, text="Price and stock alerts will appear here.",
    ...
    def build_engine_logs_panel(self, parent):
        ...
        log_area = ctk.CTkFrame(
            frame, fg_color=colors.SURFACE, border_width=1,
            border_color=colors.DIVIDER, corner_radius=7
        )
    ...
    def build_system_response_log(self, parent):
        ...
        log_container = ctk.CTkFrame(
            log_frame, fg_color=colors.SURFACE, border_width=1,
            border_color=colors.DIVIDER, corner_radius=7
        )
        log_container.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        self.log_box = ctk.CTkTextbox(
            log_container, height=100, font=fonts.LOG,
