...
class SkuPriceMonitor:
    ...
    def start(self, session: PurchaseSession):
        ...
        forensics = PromotionForensicsRecorder.get(session)
        if forensics is not None:
            forensics.record_event("monitor_started", "monitoring", {
        ...
        try:
            ...
            if state is None:
                ...
                if recorder is not None:
                    recorder.record_event("get_pc_target_not_found", "monitoring")
                return
            ...
            recorder = PromotionForensicsRecorder.get(self.session)
            if recorder is not None:
                recorder.record_event("sku_state_processed", "pdp_get_pc", {
            ...
            recorder = PromotionForensicsRecorder.get(self.session)
            if recorder is not None:
                recorder.record_event("get_pc_processing_error", "pdp_get_pc", {"error": repr(e)})
